...
К сожалению, я не смог найти достаточно надежный ответ на ваш вопрос
(уверенность только {reason.confidence_score:.0%}).

Это может означать:
• Вопрос требует анализа нескольких документов одновременно
• Ваш конкретный случай не описан в стандартных нормах
• Необходима консультация специалиста

📝 КАК ПЕРЕФОРМУЛИРОВАТЬ ВОПРОС:

Попробуйте добавить больше деталей:
✓ "Как подать жалобу на решение заказчика в течение 10 дней?"
  (вместо: "Как обжаловать решение?")

✓ "Какие документы подтверждают квалификацию поставщика?"
  (вместо: "Какие нужны документы?")

✓ "Каков штраф за неподписание договора в срок по пункту 535 ЗРГК?"
  (вместо: "Какой штраф за неподписание?")

🔗 ПОЛЕЗНЫЕ ССЫЛКИ:

Используйте команду /docs для ссылок на официальные источники:
• Законы: https://adilet.zan.kz/
• Инструкции Omarket: https://wiki.omarket.kz/
• Инструкции Goszakup: https://wiki.goszakup.gov.kz/

Если после этого остаются вопросы — обратитесь к уполномоченному органу.
""",

            "CRITICAL_HALLUCINATIONS": f"""
❌ ОШИБКА В ОТВЕТЕ - ДОКУМЕНТЫ НЕ НАЙДЕНЫ

ВНИМАНИЕ: При обработке вашего вопроса я обнаружил ошибку.
В моем ответе были упомянуты документы или статьи закона,
...
• Название документа незначительно отличается от официального
• Вопрос выходит за рамки системы госзакупок РК

🔍 ЧТО ДЕЛАТЬ:

1. Проверьте полное название нормативного документа на:
   https://adilet.zan.kz/ (основное законодательство РК)

2. Используйте команду /docs для просмотра всех поддерживаемых источников

3. Переформулируйте вопрос с точным названием документа:
   ✓ "Согласно какому пункту ЗРГК поставщик может быть исключен?"
   ✗ "Какой закон регулирует госзакупки?"

4. Если вопрос требует специального анализа — обратитесь к специалисту

📞 КОНТАКТ: Уполномоченный орган по государственным закупкам РК
""",

            "MULTIPLE_INTERPRETATIONS": f"""
❓ НЕОДНОЗНАЧНЫЙ ВОПРОС - ТРЕБУЕТСЯ УТОЧНЕНИЕ

Ваш вопрос имеет несколько возможных толкований.
В зависимости от вашей конкретной ситуации ответ может быть разным.

Я не могу выбрать один правильный вариант без дополнительной информации.

📋 УТОЧНИТЕ СЛЕДУЮЩЕЕ:

Вместо "Как добавить НДС к договору?"
✓ "У нас есть договор на 1,000,000 тенге БЕЗ НДС. Средства на НДС есть отдельно.
   Нужно ли создавать новую закупку для НДС или можно изменить существующий договор?"

Вместо "Какие документы нужны для участия?"
✓ "Поставщик - физическое лицо. Какие документы нужны для участия в открытом конкурсе?"

Вместо "Как подать жалобу?"
✓ "Я - поставщик. Заказчик выбрал другого поставщика необоснованно.
   Как мне подать жалобу в уполномоченный орган в течение 10 дней?"

🎯 ДЕТАЛИ, КОТОРЫЕ ПОМОГУТ:

• Ваша роль (заказчик или поставщик?)
• Конкретные цифры и суммы
• Конкретные сроки
• Какая платформа (Omarket или Goszakup?)
• Какой документ вас интересует

После уточнения переформулируйте вопрос и спросите снова.
""",

            "INSUFFICIENT_SOURCE_COVERAGE": f"""
📊 НЕПОЛНЫЕ РЕЗУЛЬТАТЫ ПОИСКА

К сожалению, при поиске в базе знаний найдены неполные результаты ({reason.confidence_score:.0%}).

Это может означать:
• Вопрос требует анализа нескольких нормативных актов одновременно
• Необходимо уточнить, какой конкретный документ вас интересует
• Вопрос может быть вне области госзакупок РК

✅ СЛЕДУЮЩИЕ ШАГИ:

1️⃣ Попробуйте переформулировать вопрос более конкретно:
   ✓ Указать статью/пункт закона, если вы его знаете
   ✓ Привести конкретный пример или сценарий
   ✓ Назвать конкретный документ (закон, договор, инструкция)

2️⃣ Выберите источник и спросите о нем явно:
   ✓ "Согласно ЗРГК, какой срок..."
   ✓ "На платформе Omarket, как..."
   ✓ "В Гражданском кодексе РК, где..."

3️⃣ Используйте справочник:
   Команда /docs показывает все доступные источники и ссылки
   на официальные документы

4️⃣ Если остаются вопросы:
   Обратитесь к уполномоченному органу по государственным закупкам
   или к специалисту в области закупок

💡 СОВЕТ: Чем конкретнее ваш вопрос - тем точнее мой ответ!
"""
        }
